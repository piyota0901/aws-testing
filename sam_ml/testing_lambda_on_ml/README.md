...
```bash
testing_lambda_on_ml$ sam build
```
...
```bash
testing_lambda_on_ml$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
testing_lambda_on_ml$ sam local start-api
testing_lambda_on_ml$ curl http://localhost:3000/classify_digit
```
...
```bash
testing_lambda_on_ml$ sam logs -n InferenceFunction --stack-name testing_lambda_on_ml --tail
```
...
```bash
aws cloudformation delete-stack --stack-name testing_lambda_on_ml
```
